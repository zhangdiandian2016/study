...
/**
 * @ClassName guava.cache.LoadingCacheTest
...
public class LoadingCacheTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("缓存是否存在=" + cache.getIfPresent("name").hashCode());
        //休眠
        Thread.sleep(500);
        //System.out.println("2秒后"+"，时间="+new Date());
        System.out.println("0.5秒后，缓存是否存在=" + cache.getIfPresent("name").hashCode());
        //启动3个线程
        for (int i = 0; i < 3; i++) {
            startThread(i);
        }
        ...
        //模拟串行读缓存
        Thread.sleep(5000);
        System.out.println(Thread.currentThread().getName() + "休眠5秒后，读缓存="+cache.get("name").hashCode()+"，时间=" + new Date());
        Thread.sleep(500);
        System.out.println(Thread.currentThread().getName() + "距离上一次读0.5秒后，读缓存="+cache.get("name").hashCode()+"，时间=" + new Date());
        Thread.sleep(700);
        System.out.println(Thread.currentThread().getName() + "距离上一次读0.5秒后，读缓存="+cache.get("name").hashCode()+"，时间=" + new Date());
    }
        ...
        Thread t = new Thread(new Runnable() {
            public void run() {
                try {
                    ...
                    //读缓存
                    System.out.println(
                        Thread.currentThread().getName() + "并发读缓存=" + cache.get("name").hashCode() + "，时间=" + new Date());
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
